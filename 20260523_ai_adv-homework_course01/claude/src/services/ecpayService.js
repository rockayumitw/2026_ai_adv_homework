...
// 回傳 { action, params }
function buildPaymentParams({ order, orderItems, serverBaseUrl }) {
  ...
  const params = {
    ...
    EncryptType: '1',
    // 付款完成後瀏覽器自動導回（Form POST），用於觸發驗證
    OrderResultURL: `${serverBaseUrl}/api/ecpay/order-result`,
    // 用戶按「取消/返回」按鈕時導回訂單頁
    ClientBackURL: `${serverBaseUrl}/orders/${order.id}`,
  };
